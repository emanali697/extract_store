...
from config import (
    ...
    SIGN_ZOOM_FACTOR, GPS_ZOOM_FACTOR, SIGN_CROP_Y_RANGE,
...
def enhance_image(img):
    """تحسين جودة الصورة - contrast + sharpness"""
...
def extract_sign_crop(frame):
    """قص منطقة اللوحات من الإطار"""
    h, w = frame.shape[:2]
    y1 = int(h * SIGN_CROP_Y_RANGE[0])
    y2 = int(h * SIGN_CROP_Y_RANGE[1])
    sign_crop = frame[y1:y2, 0:w]
    enhanced = enhance_image(sign_crop)
    enlarged = cv2.resize(enhanced, None, fx=SIGN_ZOOM_FACTOR, fy=SIGN_ZOOM_FACTOR,
                          interpolation=cv2.INTER_CUBIC)
    return enlarged
...
def filter_frames_by_speed(raw_frames, gps_data_list, speed_mode="auto", log_fn=print):
    ...
    - نختار الفريمات بفترة تناسب السرعة
    """
    if not raw_frames:
        return []
    ...
    for i, frame_info in enumerate(raw_frames):
        ...
        # اختيار الفترة المناسبة للسرعة
        if speed_mode == "fast":
            required_interval = 1.0
        elif speed_mode == "precise":
            required_interval = 0.5
        else:
            required_interval = get_interval_for_speed(speed)
        if timestamp - last_selected_time < required_interval - 0.01:
            skipped_interval += 1
            continue

        selected.append({**frame_info, **gps})
    ...
    if not selected and raw_frames:
        ...
        for i, frame_info in enumerate(raw_frames):
            timestamp = frame_info["timestamp"]
            if timestamp - last_selected_time < fallback_interval - 0.01:
                continue
            gps = gps_data_list[i] if i < len(gps_data_list) else {}
            selected.append({**frame_info, **gps})
            last_selected_time = timestamp
